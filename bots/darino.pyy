...
def bind_request_code():
    """ Keeps working as before - requests the code from Darino """
    try:
        user_id = get_user_id_from_token(request)
        if not user_id: return jsonify({"success": False, "error": "Unauthorized"}), 401
        ...
        if phone_res.get("code") == 0:
            ...
            return jsonify({
                "success": True, 
                "uuid": uuid_val, 
                "phone_code": phone_res.get("data", {}).get("phone_code", "777777")
            })
...
def bind_check_status():
    """ 
    UPDATED: Force success logic. 
    Doesn't care about UUID/Token check, just updates status to bound.
    """
    try:
        user_id = get_user_id_from_token(request)
        if not user_id: return jsonify({"success": False, "error": "Unauthorized"}), 401
        ...
        try:
            ...
            logging.error(f"Supabase status update failed: {e}")
            # Even if DB fails, we will return success to the UI

        return jsonify({
            "success": True, 
            "message": "Account marked as bound!"
        })

    except Exception as e:
        logging.error(f"Status route error: {e}")
        return jsonify({"success": True}) # Return success anyway to close modal
