...
def bind_check_status():
    try:
        ...
        # 1. Capture the UUID sent by the frontend request body
        request_uuid = data.get("uuid") 

        if not account_id:
            return jsonify({"success": False, "error": "Missing account_id"}), 400

        # Fetch account from DB to get the token
        accounts = get_user_accounts(user_id, bot_type="darino") or []
        account = next((a for a in accounts if a.get("id") == account_id), None)
        ...
        # 2. Get the token (needed for Darino API)
        token = account.get("token")
        
        # 3. PRIORITY LOGIC: Use UUID from request first, fallback to DB
        final_uuid = request_uuid or account.get("uuid")

        # FIX: Only error if BOTH are missing
        if not token or not final_uuid:
            logging.error(f"Missing session info: Token exists? {bool(token)}, UUID exists? {bool(final_uuid)}")
            return jsonify({"success": False, "error": "Binding session missing - Token or UUID not found"}), 400

        # 4. Call Darino API with the uuid we got from the payload
        scan_res = scan_result(final_uuid, token)
        
        if scan_res.get("code") == 0:
            # Only if it succeeds, update the DB status to 'bound'
            try:
                save_bot_accounts(user_id, [{"id": account_id, "status": "bound"}], update=True)
            except:
                pass # Non-critical if DB fails here
